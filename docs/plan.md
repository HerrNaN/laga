...
2.  **Sync Strategy:** Client-overwrites-server (Last-Write-Wins) for the MVP. Multi-device conflict resolution is deferred to v2.
3.  **Backend as Static File Server:** The Go backend will serve the compiled frontend (`frontend/dist/`) in production. Separate dev processes will be used during development.
4.  **Styling:** Plain CSS for the MVP to keep dependencies minimal.
5.  **Database Access:** `sqlc` generates type-safe Go code from annotated SQL queries. We write SQL; sqlc writes the Go.
6.  **Migrations:** `goose` manages schema changes as versioned, single-file SQL migrations.
7.  **Schema Source:** `backend/schema.sql` is a generated file (not hand-written). A Go script (`scripts/generate-schema.go`) spins up a temporary PostgreSQL container, applies all migrations via goose, and dumps the resulting schema via `pg_dump --schema-only`. sqlc then reads `schema.sql` for type inference.
8.  **Task Runner:** `mise` manages all dev tools (Go, Node, sqlc, goose) and defines project tasks inline in `mise.toml` (no dot prefix).
...
| **Dev Tools** | `mise` (`mise.jdx.dev`) | Manages `go`, `node`, `sqlc`, `goose` versions and project tasks. |
...
│   ├── main.go                -- Server boot, routes, CORS, static file serving
...
[env]
DATABASE_URL = "postgres://user:pass@localhost:5432/laga?sslmode=disable"
...
depends = ["dev-be", "dev-fe"]
```

## Implementation Roadmap

### Phase 0: Scaffold & Tooling
1.  Write `mise.toml` pinning Go, Node, sqlc, and goose versions. Define all project tasks.
2.  Initialize `backend/` as a Go module.
3.  Initialize `frontend/` with the official Vite Svelte template.
4.  Install `vite-plugin-pwa` and configure `vite.config.js` to generate a Service Worker that precaches the app shell.
...
7.  Run `mise run generate-schema` and verify `backend/schema.sql` is created.
...
6.  Add minimal CORS middleware allowing requests from `http://localhost:5173`.
7.  Run `mise run migrate` to apply the initial migration to your local PostgreSQL.
